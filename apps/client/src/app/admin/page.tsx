...
export default function AdminDashboard() {
  ...
  const { stats: aiLeadsStats, isLoading: aiLeadsStatsLoading } = useAILeadsStats();
  const { leads: recentAILeads, isLoading: recentAILeadsLoading } = useRecentAILeads(5);
    ...
    <div className="min-h-screen">
      ...
      <main className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 mb-6">
          ...
          <MetricCard
            title="Chatbot Leads"
            value={aiLeadsStatsLoading ? '...' : (aiLeadsStats?.totalLeads || 0)}
            change={aiLeadsStats?.growth || 0}
            changeLabel="vs last 30d"
          />
          <MetricCard
            title="Total Revenue"
        ...
        <div className="bg-white rounded-xl border border-slate-200 mb-6">
          <div className="p-5 border-b border-slate-200">
            <h3 className="text-lg font-semibold text-slate-900">Recent Chatbot Leads</h3>
          </div>
          {recentAILeadsLoading ? (
            <div className="p-5">
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="animate-pulse h-12 bg-slate-100 rounded-lg" />
          ...
          ) : recentAILeads && recentAILeads.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50">
                    <th className="text-left px-5 py-3 text-xs font-medium text-slate-500 uppercase">Email</th>
                    <th className="text-left px-5 py-3 text-xs font-medium text-slate-500 uppercase">Messages</th>
            ...
            <div className="p-8 text-center text-slate-500">
              No chatbot leads yet - visitors who chat will appear here
            </div>
          )}
        </div>
